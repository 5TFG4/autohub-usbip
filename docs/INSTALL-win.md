...
Edit `clients.allow` so it lists every source allowed to talk to the local HTTP listener (at minimum the Pi's static IP). One IPv4 or CIDR per line; `#` starts a comment.

## 3. HTTP Listener (Port 59876)

`listener.ps1` hosts a lightweight `HttpListener` on `http://+:59876/usb-event/`. The script:
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File C:\Autohub\listener.ps1 -Pi 192.168.1.2 -Prefix "http://+:59876/usb-event/"
```
...
`sync.ps1` continuously aligns local `usbip` attachments with whatever the Pi exports:
...
You can run it manually during testing:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File C:\Autohub\sync.ps1 -Pi 192.168.1.2
```
...
$listenerAction = New-ScheduledTaskAction -Execute "powershell.exe" `
  -Argument "-NoProfile -ExecutionPolicy Bypass -File C:\Autohub\listener.ps1 -Pi $pi"
...
$syncAction = New-ScheduledTaskAction -Execute "powershell.exe" `
  -Argument "-NoProfile -ExecutionPolicy Bypass -File C:\Autohub\sync.ps1 -Pi $pi"
...
- Update `C:\Autohub\clients.allow` whenever client IPs change, then re-run `update-firewall.ps1`.
